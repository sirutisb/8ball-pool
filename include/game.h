...
public:
    NumberBall(BallType type, int number) : Ball(type), number_(number) {}
...
private:
    ...
    void render() {
        ...
        static sf::CircleShape circ(50);
        circ.setOrigin(sf::Vector2f(50.f, 50.f));
        circ.setFillColor(sf::Color::Green);

        static sf::Vector2f tableSize{1500.f, 750.f};
        static sf::RectangleShape table(tableSize);
    ...
    for (int col = 0; col < 5; ++col) {
        for (int row = 0; row <= col; ++row) {
            ...
            ball.setPosition(sf::Vector2f{apexPos.x + x, apexPos.y + y});
            ...
            if (col == 2 && row == 1) {
                ...
                ball.setFillColor(sf::Color::White);   // Rest are cue/random
            }
        ...
        window_.clear(sf::Color(90, 90, 90));
